...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = [JKUtil getColor:@"f3f3f3"];
...
//    self.navigationItem.titleView = self.titleButton;
    ...
    titleView.textColor = [BrowserManager shareInstance].configure.titleColor;
    titleView.textAlignment = NSTextAlignmentCenter;
    [titleView sizeToFit];
    self.navigationItem.titleView = titleView;
    
    self.navigationController.navigationBar.translucent = NO;
    self.navigationController.navigationBar.barTintColor = [BrowserManager shareInstance].configure.navigationBackgroundColor;
    
    UIButton *preBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    preBtn.frame = CGRectMake(0, 0, 40, 20);
    [preBtn setTitle:@"确定" forState:UIControlStateNormal];
    [preBtn setTitleColor:[BrowserManager shareInstance].configure.rightButtonTitleColor forState:UIControlStateNormal];
    [preBtn setTitleColor:[[BrowserManager shareInstance].configure.rightButtonTitleColor colorWithAlphaComponent:.5f] forState:UIControlStateDisabled];
    [preBtn setTitleColor:[[BrowserManager shareInstance].configure.rightButtonTitleColor colorWithAlphaComponent:.5f] forState:UIControlStateHighlighted];
                         ...
                         completion:^(NSArray *assetsGroups) {
                             if ([assetsGroups count]>0) {
                                 ...
                                 dispatch_async(dispatch_get_main_queue(), ^{
                                     ...
                                     [weakSelf.collectionView scrollToBottomAnimated:NO];
...
- (void)setShowsCancelButton:(BOOL)showsCancelButton{
    ...
    if (showsCancelButton) {
        ...
        [cancelBtn setTitleColor:[BrowserManager shareInstance].configure.leftButtonTitleColor forState:UIControlStateNormal];
        [cancelBtn setTitleColor:[[BrowserManager shareInstance].configure.leftButtonTitleColor colorWithAlphaComponent:.5f] forState:UIControlStateHighlighted];
...
#define kSizeThumbnailCollectionView  ([UIScreen mainScreen].bounds.size.width-42)/3
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat columnMargin = [BrowserManager shareInstance].configure.itemColumnMargin;      // 列间距
    
    UIEdgeInsets sectionInsets = [BrowserManager shareInstance].configure.sectionInsets;
    
    NSInteger column = [BrowserManager shareInstance].configure.column;
    
    CGFloat itemWH = (screenW - sectionInsets.left - sectionInsets.right - (columnMargin) * (column - 1)) / column;
...
{
    return  [BrowserManager shareInstance].configure.sectionInsets;
}


#pragma mark - 👀 选中某个图片的回调 👀 💤

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
     JKAssetsViewCell *assetsCell = (JKAssetsViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
    
    if (assetsCell.checkButton.selected)
    {
        [self didDeselectItemAssetsViewCell:assetsCell];
    }
    else
    {
        [self didSelectItemAssetsViewCell:assetsCell];
    }
...
- (void)photoBrowser:(JKPhotoBrowser *)photoBrowser didSelectAtIndex:(NSInteger)index
{
    ...
    if (assetURL){
       ...
       image = [JKUtil imageNamed:@"assets_placeholder_picture"];
    }
    ...
    [[PhotoAlbumManager sharedManager] saveImage:image
                                 ...
                                 completionBlock:^(ALAsset *asset, NSError *error) {
                                     if (error == nil && asset) {
                                         NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
                                         [self addAssetsObject:assetURL realImg:image];
                                         [weakSelf.assetsArray insertObject:asset atIndex:0];
                                         [weakSelf resetFinishFrame];
...
- (void)addAssetsObject:(NSURL *)assetURL realImg:(UIImage *)img
{
    ...
    ALAssetsLibrary *lib = [[ALAssetsLibrary alloc] init];
            ...
            [lib assetForURL:assetURL resultBlock:^(ALAsset *myasset){
                if (myasset)
                {
                    CGImageRef cgImg = [[myasset defaultRepresentation] fullScreenImage];
                    
                    UIImage *img = [UIImage imageWithCGImage:cgImg];
                    
                    if (img && [img isMemberOfClass:[UIImage class]])
                    {
                        asset.photo = img;
                    }
                }
...
- (void)resetFinishFrame
{
    ...
    {
        NSString *enterTitle = [NSString stringWithFormat:@"确认(%zi)", self.selectedAssetArray.count];
...
- (UIButton *)titleButton{
    if (!_titleButton) {
        ...
        UIImage  *img =[JKUtil imageNamed:@"navigationbar_title_highlighted"];
...
{
    if (!_arrowImageView) {
        UIImage  *img = [JKUtil imageNamed:@"navigationbar_arrow_down"];
        UIImage  *imgSelect = [JKUtil imageNamed:@"navigationbar_arrow_up"];
...
- (UIToolbar *)toolbar
{
    if (!_toolbar) {
        ...
        UIImage  *img = [JKUtil imageNamed:@"compose_photo_original"];
        UIImage  *imgSelect = [JKUtil imageNamed:@"compose_photo_original_highlighted"];
        ...
        [_finishButton setBackgroundImage:[JKUtil stretchImage:[JKUtil imageNamed:@"picker_button_orange"] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateNormal];
        [_finishButton setBackgroundImage:[JKUtil stretchImage:[JKUtil imageNamed:@"picker_button_orange_highlighted"] capInsets:UIEdgeInsetsMake(5, 5, 5, 5) resizingMode:UIImageResizingModeStretch] forState:UIControlStateHighlighted];
