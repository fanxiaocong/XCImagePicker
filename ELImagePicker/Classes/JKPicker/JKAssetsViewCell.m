...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self imageView];
        self.checkButton.right = self.imageView.width-3;
...
- (void)setAsset:(ALAsset *)asset
{
    ...
    if (_asset != asset) {
        ...
        if (thumbnailImageRef) {
            ...
            self.imageView.image = [JKUtil imageNamed:@"assets_placeholder_picture"];
        }
    }
}
...
- (UIButton *)checkButton
{
    if (!_checkButton)
    {
        ...
        UIImage *img = [JKUtil imageNamed:@"photo_check_default"];
        UIImage *imgH = [JKUtil imageNamed:@"photo_check_selected"];
        
        _checkButton.frame = CGRectMake(0, 3, img.size.width, img.size.height);
...
- (UIButton *)photoButton
{
    if (!_photoButton) {
        ...
        UIImage  *bgImg = [JKUtil imageNamed:@"compose_photograph_background"];
        [_photoButton setBackgroundImage:bgImg forState:UIControlStateNormal];
        [_photoButton setBackgroundImage:bgImg forState:UIControlStateNormal];

        UIImage *img = [JKUtil imageNamed:@"compose_photograph"];
        UIImage *imgH = [JKUtil imageNamed:@"compose_photograph_highlighted"];
